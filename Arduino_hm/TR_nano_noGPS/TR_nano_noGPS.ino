...
float listCalibMAG[3] = {0, 0, 0};                          // factory mag calibration and mag bias
...
void setup()
{
  ...
  int calibMAGx = listCalibMAG[0]*10000;
  int calibMAGy = listCalibMAG[1]*10000;
  int calibMAGz = listCalibMAG[2]*10000;

    
  Serial.println("Magnetometer sensitivity adjustement values (G):");
  Serial.print("X = "); Serial.println(listCalibMAG[0], 4);
  Serial.print("Y = "); Serial.println(listCalibMAG[1], 4);
  Serial.print("Z = "); Serial.println(listCalibMAG[2], 4);
  Serial.println();

  // Transmit calculation parameters
  Serial.println("---Parameters transmission---");
  
  byte counter = 0;
  int count = 0;
  byte oldByte = 0;
  int array2send[6] = {aResOut, gResOut, mResOut, calibMAGx, calibMAGy, calibMAGz};
  while(count < 6){          
    printf("Now sending %d as payload. ",(int)array2send[counter]); 
...
void loop()
{
  ...
  if (I2CreadByte(MPU9250_ADDRESS, INT_STATUS) & 0x01)
  {
    ...
    int16_t dataOut[9] = {accelData[0], accelData[1], accelData[2], gyroData[0], gyroData[1], gyroData[2], magData[0], magData[1], magData[2]};
