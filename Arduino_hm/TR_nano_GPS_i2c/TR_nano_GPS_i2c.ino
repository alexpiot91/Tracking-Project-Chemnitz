 ...
 *  - https://tmrh20.github.io/RF24/pingpair_ack_8ino-example.html
 */
...
SFE_UBLOX_GPS myGPS;
long latitude, longitude, speed, heading, SIV;
int lat_int, lat_dec, lng_int, lng_dec, vel_int, head_int, alt_int;
...
void initMAG(float * destination)
{
  ...
  uint8_t rawData[3];  // x, y, z mag sensitivity values stored here
...
// GPS treatment and sending functions
void coordIntFormat(int &leftInt, int &rightInt, long coord)
{
  float coord_temp = (float)coord/100000.0;
  leftInt = floor(coord_temp);
  rightInt = (coord_temp-leftInt)*10000;
}


int speedIntFormat(long v)
{
  int v_int = floor((float)v/10.0); // in cm/s
  return v_int;
}


int headIntFormat(long h)
{
  int h_int = floor((float)h/10000.0); // in tenth of °
  return h_int;
}


int altIntFormat(long a)
{
  int h_int = floor((float)a/1000.0); // in m
  return h_int;
}
...
void setup()
{
  // Initialization of communication (Serial, I2C, GPS)
  Serial.begin(115200);
  while (!Serial)
  Serial.println(F("---I2C and RF initialization---"));
  Wire.begin();
  ...
  myGPS.setAutoPVT(true);                // enable automatic PVT (Position,Velocity,Time etc) reports at the navigation frequency
  myGPS.saveConfiguration();             // save the current settings to flash and BBR

  byte rate = myGPS.getNavigationFrequency();  // check the update rate of this module
  ...
  Serial.println(F("---Parameters RF transmission---"));
  getAres();
  getGres();
  getMres();
  byte counter = 0;
  ...
  while(count < 3){   
    Serial.print(F("Sending: ")); Serial.print((int)array2send[counter]);
    ...
    if (!radio.write( &data2send, sizeof(data2send) )){
      ...
      if(!radio.available()){ 
        ...
        while(radio.available() ){
          ...
          Serial.print(F(", Response: ")); Serial.println(ackByte);
          if(ackByte != oldByte){
            oldByte = ackByte;
            count++;
            counter++;
...
void loop()
{
  if (millis() - lastTime2 > 5)
  {
    if (millis() - lastTime1 > 100)
    {
      ...
      // GPS output are long values so we need to cut them into two integers that can be sent in our RF payload.
