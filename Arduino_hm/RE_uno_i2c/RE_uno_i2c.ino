...
void setup()
{
  // Initialization of communication (Serial)
  Serial.begin(115200);
  while (!Serial)
  Serial.println("---RF initialization---");
  ...
  Serial.println("---Parameters RF transmission---");
  int count = 0;
  int oldByte = 0;
  byte ackByte = 1;
  int array2receive[3];
...
void loop()
{
  ...
  if (radio.available())
  {
    ...
    if (dataIn[9]!= 0)
    {
      ...
      Serial.print(0);  
      Serial.print (";");
      Serial.print((float)dataIn[13]/10.0);  // in °
      Serial.print (",");
      Serial.print((float)dataIn[14]/100.0);  // in m/s
      Serial.print (",");
      Serial.print(dataIn[15]);  // in m
      }
